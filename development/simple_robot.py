...
#       In de documentatie staat dat de SONAR HC-SR04 de afstand in meters returned.
#       Met de testing die we tot nu toe hebben gedaan lijken deze sensoren niet al te accuraat though :(
# 1b Kan je een overzichtje geven hoe dit werkt? Krijgen we afstand in meters uit get_right_distance?
#       Basically kan je de python API van Mirte gebruiken voor de basic verwerking van sensordata.


# 2 Welke functies kunnen we qua sensor input (afstand, IR en camera) nog meer toevoegen?
#       We gaan vandaag proberen de LIDAR scanner nog toe te voegen en een inputsignaal te ontvangen.
#       Het is wel een feit dat ik echt daadwerkelijk geen flauw idee heb hoe dit signaal eruit gaat zien.
