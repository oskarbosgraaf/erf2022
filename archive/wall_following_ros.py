...
# Util imports
import randomfrom geometry_msgs.msg import Twist
import math
import time
...
def clbk_laser(msg):
    ...
    # Determination of minimum distances in each region
    regions = {
        'bleft' : min(min(msg.ranges[90:126]), self.inf),
        'left': min(min(msg.ranges[54:90]),self.inf),
        'fleft':min(min(msg.ranges[18:54]),self.inf),
        'front':min(min(min(msg.ranges[0:18]), min(msg.ranges[342:359])), self.inf),
        'fright':min(min(msg.ranges[306:342]), self.inf),
        'right': min(min(msg.ranges[270:306]), self.inf),
        'bright': min(min(msg.ranges[234:270]), self.inf)
        }
    #rospy.loginfo(regions_)
...
def random_wandering():
    ...
    msg = Twist()
    msg.linear.x = float(max(min( last_vel[0] + random.uniform(-0.01,0.01),0.3),0.1))
    msg.angular.z= float(max(min( last_vel[1] + random.uniform(-0.1,0.1),1),-1))
...
def main():
    ...
    pub_ = node.create_publisher(Twist, 'cmd_vel', qos)
    sub = node.create_subscription(LaserScan, 'scan', clbk_laser,  qos_profile = qos_profile_sensor_data)
    
    
    print('Code is running')
    # rate = rclpy.timer.Rate(hz)
